...
TEST(FTPClientTest, ConnectToServer) {
    ...
    EXPECT_CALL(mockFtpClient, connect("example.com", 21)).Times(1);

    try {
        mockFtpClient.connect("example.com", 21);
...
TEST(FTPClientTest, Authenticate) {
    ...
    EXPECT_CALL(mockFtpClient, connect("example.com", 21)).Times(1);
    EXPECT_CALL(mockFtpClient, authenticate("validUsername", "validPassword")).Times(1);

    try {
        mockFtpClient.connect("example.com", 21);
        mockFtpClient.authenticate("validUsername", "validPassword");
        ASSERT_TRUE(mockFtpClient.isConnected());
    } catch (const std::exception& e) {
...
TEST(FTPClientTest, DownloadFile) {
    ...
    EXPECT_CALL(mockFtpClient, connect("example.com", 21)).Times(1);
    EXPECT_CALL(mockFtpClient, downloadFile("/remote/path/to/file.txt", "./local_file.txt")).Times(1);

    try {
        mockFtpClient.connect("example.com", 21);
        mockFtpClient.downloadFile("/remote/path/to/file.txt", "./local_file.txt");
        ASSERT_TRUE(mockFtpClient.isConnected());
    } catch (const std::exception& e) {
...
TEST(FTPClientTest, UploadFile) {
    ...
    EXPECT_CALL(mockFtpClient, connect("example.com", 21)).Times(1);
    EXPECT_CALL(mockFtpClient, uploadFile("./local_file.txt", "/remote/path/to/file.txt")).Times(1);

    try {
        mockFtpClient.connect("example.com", 21);
        mockFtpClient.uploadFile("./local_file.txt", "/remote/path/to/file.txt");
        ASSERT_TRUE(mockFtpClient.isConnected());
    } catch (const std::exception& e) {
